...
let machadoModel = null;

loader.load(
  './machadoV1_LoRes.glb',
  function (gltf) {
    machadoModel = gltf.scene;
    machadoModel.position.set(0, -.45, .05);
    machadoModel.scale.set(1, 1, 1); // ajuste se necessário
    scene.add(machadoModel);
  },
  undefined,
  function (error) {
...
function animate() {
  controls.update();
  if (machadoModel) {
    machadoModel.rotation.y += 0.01; // gira suavemente no eixo Y
  }
